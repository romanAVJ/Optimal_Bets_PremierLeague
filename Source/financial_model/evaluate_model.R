...
# init params -------------------------------------------------------------
FILE_BETS <- "Results/bets/ridge_one_kelly.csv"
...
df_main_bets <- df_r %>% 
  mutate(
    ...
    theoric_return = exp(log_growth * FRACTIONAL_KELLY)
  ) 
...
ts_wealth <- df_main_bets %>% 
  ...
  summarise(
    wealth_return = 1 + sum(fractional_return, na.rm = TRUE)
  ...
  mutate(
    ...
    # theoric_final_wealth = cumprod(theoric_return, na.rm = TRUE)
  ) %>% 
  add_row(
...
ts_wealth %>% 
  ...
  scale_y_continuous(labels = scales::percent, limits = c(-1, 1.30)) +
  theme_bw() +
  theme(
...
ggsave(
  filename = str_glue("one_kelly{FRACTIONAL_KELLY}.png"), 
  device = png(), 
